...
fn test_airmass_consistency() {
    ...
    for alt in altitudes {
        let am_pp = airmass_plane_parallel(alt);
        let am_young = airmass_young(alt);
        let am_pickering = airmass_pickering(alt);
        let am_ky = airmass_kasten_young(alt);
...
fn test_airmass_increases_with_zenith_angle() {
    ...
    for window in altitudes.windows(2) {
        let am1 = airmass_pickering(window[0]);
        let am2 = airmass_pickering(window[1]);
...
fn test_extinction_wavelength_dependence() {
    ...
    for window in wavelengths.windows(2) {
        let k1 = extinction_coefficient_estimate(window[0].0);
        let k2 = extinction_coefficient_estimate(window[1].0);
